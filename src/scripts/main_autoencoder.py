"""
Script principal para experimentos com o modelo Autoencoder.
...
def main():
    """Executa o pipeline completo de experimentos com Autoencoder."""
    results_dir = get_results_dir("autoencoder")
    ...
    history_path = os.path.join(results_dir, "autoencoder_histories.pkl")
    ...
    repr_path = os.path.join(results_dir, "autoencoder_representations.npz")
    np.savez_compressed(repr_path, **all_repr)
    print(f"  Representations saved: {repr_path}")

    save_tsne_embeddings(all_repr, model_name="autoencoder")
    ...
    print(f"  python src/analysis/visualize_results.py autoencoder")
