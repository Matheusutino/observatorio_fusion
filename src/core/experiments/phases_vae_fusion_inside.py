"""
Lógica das fases experimentais para VAE com fusão interna.

Similar a phases_autoencoder_fusion_inside.py, mas adaptado para VAE.
"""
import itertools
import numpy as np
import pandas as pd
...
def run_vae_fusion_inside_experiment(
    op_names: list[str],
    emb_prod: np.ndarray,
    emb_tema: np.ndarray,
    y: np.ndarray,
    ...
    """
    Treina e avalia um VAE com fusão interna usando validação cruzada.
    """
    label = "+".join(op_names)
    ...
    trainer_kwargs={"lambda_class": lambda_class, "lambda_recon": lambda_recon, "beta": beta},
    train_verbose=True,
    )


def run_phase1_vae_fusion_inside(
    emb_prod: np.ndarray,
    emb_tema: np.ndarray,
    y: np.ndarray,
    model_class,
    model_kwargs: dict,
    le: LabelEncoder,
    model_name: str = "vae_fusion_inside",
    lambda_class: float = 1.0,
    lambda_recon: float = 1.0,
    beta: float = 1.0,
    ...
    for op_name in FUSION_OPS:
        res = run_vae_fusion_inside_experiment(
            [op_name], emb_prod, emb_tema, y,
            model_class, model_kwargs, le,
            lambda_class, lambda_recon, beta
        )
...
def run_phase2_vae_fusion_inside(
    emb_prod: np.ndarray,
    emb_tema: np.ndarray,
    y: np.ndarray,
    model_class,
...
def run_phase3_vae_fusion_inside(
