"""
Lógica das fases experimentais para Encoder com fusão interna.

Similar a phases_autoencoder_fusion_inside.py, mas adaptado para modelos de classificação apenas
(sem reconstrução).
"""
import itertools
import numpy as np
...
def run_encoder_fusion_inside_experiment(
    op_names: list[str],
    emb_prod: np.ndarray,
    emb_tema: np.ndarray,
    y: np.ndarray,
...
def run_phase1_encoder_fusion_inside(
    emb_prod: np.ndarray,
    emb_tema: np.ndarray,
    y: np.ndarray,
    model_class,
    ...
    for op_name in FUSION_OPS:
        res = run_encoder_fusion_inside_experiment(
...
def run_phase2_encoder_fusion_inside(
    emb_prod: np.ndarray,
    emb_tema: np.ndarray,
    y: np.ndarray,
    model_class,
...
def run_phase3_encoder_fusion_inside(
