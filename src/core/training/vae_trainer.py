"""
Trainer específico para VAE com triple loss.
"""
import torch
...
class VAETrainer:
    """
    Trainer para VAE com triple loss: reconstruction + KL divergence + classification.
    ...
    def train_epoch(self, loader: DataLoader) -> tuple[float, float, float, float]:
        ...
        for X_batch, y_batch in loader:
            ...
            # Forward pass
            logits, reconstruction, mu, logvar = self.model(X_batch)

            # Compute losses
            loss_class = self.criterion_class(logits, y_batch)
            loss_recon = self.criterion_recon(reconstruction, X_batch)
